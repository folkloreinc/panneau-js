flklrJsonp([ 119 ], {
    86: function(_, e) {
        ace.define("ace/mode/lsl_highlight_rules", [ "require", "exports", "module", "ace/lib/oop", "ace/mode/text_highlight_rules" ], function(_, e, l) {
            "use strict";
            ...
            var E = _("../lib/oop"), A = _("./text_highlight_rules").TextHighlightRules;
            E.inherits(LSLHighlightRules, A), e.LSLHighlightRules = LSLHighlightRules;
        }), ace.define("ace/mode/matching_brace_outdent", [ "require", "exports", "module", "ace/range" ], function(_, e, l) {
            "use strict";
            var E = _("../range").Range, A = function() {};
            (function() {
                this.checkOutdent = function(_, e) {
                    return !!/^\s+$/.test(_) && /^\s*\}/.test(e);
                }, this.autoOutdent = function(_, e) {
                    var l = _.getLine(e), A = l.match(/^(\s*\})/);
                    if (!A) return 0;
                    var T = A[1].length, R = _.findMatchingBracket({
                        row: e,
                        column: T
                    });
                    if (!R || R.row == e) return 0;
                    var t = this.$getIndent(_.getLine(R.row));
                    _.replace(new E(e, 0, e, T - 1), t);
                }, this.$getIndent = function(_) {
                    return _.match(/^\s*/)[0];
                };
            }).call(A.prototype), e.MatchingBraceOutdent = A;
        }), ace.define("ace/mode/folding/cstyle", [ "require", "exports", "module", "ace/lib/oop", "ace/range", "ace/mode/folding/fold_mode" ], function(_, e, l) {
            "use strict";
            var E = _("../../lib/oop"), A = _("../../range").Range, T = _("./fold_mode").FoldMode, R = e.FoldMode = function(_) {
                _ && (this.foldingStartMarker = new RegExp(this.foldingStartMarker.source.replace(/\|[^|]*?$/, "|" + _.start)), 
                this.foldingStopMarker = new RegExp(this.foldingStopMarker.source.replace(/\|[^|]*?$/, "|" + _.end)));
            };
            E.inherits(R, T), function() {
                ...
                this.getFoldWidget = function(_, e, l) {
                    var E = _.getLine(l);
                    if (this.singleLineBlockCommentRe.test(E) && !this.startRegionRe.test(E) && !this.tripleStarBlockCommentRe.test(E)) return "";
                    var A = this._getFoldWidgetBase(_, e, l);
                    return !A && this.startRegionRe.test(E) ? "start" : A;
                }, this.getFoldWidgetRange = function(_, e, l, E) {
                    var A = _.getLine(l);
                    if (this.startRegionRe.test(A)) return this.getCommentRegionBlock(_, A, l);
                    var T = A.match(this.foldingStartMarker);
                    if (T) {
                        var R = T.index;
                        if (T[1]) return this.openingBracketBlock(_, T[1], l, R);
                        var t = _.getCommentFoldRange(l, R + T[0].length, 1);
                        return t && !t.isMultiLine() && (E ? t = this.getSectionRange(_, l) : "all" != e && (t = null)), 
                        t;
                    }
                    if ("markbegin" !== e) {
                        var T = A.match(this.foldingStopMarker);
                        if (T) {
                            var R = T.index + T[0].length;
                            return T[1] ? this.closingBracketBlock(_, T[1], l, R) : _.getCommentFoldRange(l, R, -1);
                        }
                    }
                }, this.getSectionRange = function(_, e) {
                    var l = _.getLine(e), E = l.search(/\S/), T = e, R = l.length;
                    e += 1;
                    for (var t = e, S = _.getLength(); ++e < S; ) {
                        l = _.getLine(e);
                        var L = l.search(/\S/);
                        if (-1 !== L) {
                            if (E > L) break;
                            ...
                            if (I) {
                                ...
                                if (I.isMultiLine()) e = I.end.row; else if (E == L) break;
                            }
                            t = e;
                        }
                    }
                    ...
                    for (var E = e.search(/\s*$/), T = _.getLength(), R = l, t = /^\s*(?:\/\*|\/\/|--)#?(end)?region\b/, S = 1; ++l < T; ) {
                        e = _.getLine(l);
                        var L = t.exec(e);
                        if (L && (L[1] ? S-- : S++, !S)) break;
                    }
                    var I = l;
                    if (I > R) return new A(R, E, I, e.length);
                };
            }.call(R.prototype);
        }), ace.define("ace/mode/lsl", [ "require", "exports", "module", "ace/mode/lsl_highlight_rules", "ace/mode/matching_brace_outdent", "ace/range", "ace/mode/text", "ace/mode/behaviour/cstyle", "ace/mode/folding/cstyle", "ace/lib/oop" ], function(_, e, l) {
            "use strict";
            var E = _("./lsl_highlight_rules").LSLHighlightRules, A = _("./matching_brace_outdent").MatchingBraceOutdent, T = (_("../range").Range, 
            _("./text").Mode), R = _("./behaviour/cstyle").CstyleBehaviour, t = _("./folding/cstyle").FoldMode, S = _("../lib/oop"), L = function() {
                this.HighlightRules = E, this.$outdent = new A(), this.$behaviour = new R(), this.foldingRules = new t();
            };
            S.inherits(L, T), function() {
                this.lineCommentStart = [ "//" ], this.blockComment = {
                    start: "/*",
                    end: "*/"
                }, this.getNextLineIndent = function(_, e, l) {
                    var E = this.$getIndent(e), A = this.getTokenizer().getLineTokens(e, _), T = A.tokens;
                    A.state;
                    if (T.length && "comment.block.lsl" === T[T.length - 1].type) return E;
                    if ("start" === _) {
                        e.match(/^.*[\{\(\[]\s*$/) && (E += l);
                    }
                    return E;
