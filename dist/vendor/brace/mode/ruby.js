flklrJsonp([ 90 ], {
    121: function(e, t) {
        ace.define("ace/mode/ruby_highlight_rules", [ "require", "exports", "module", "ace/lib/oop", "ace/mode/text_highlight_rules" ], function(e, t, n) {
            "use strict";
            var r = e("../lib/oop"), s = e("./text_highlight_rules").TextHighlightRules, a = t.constantOtherSymbol = {
                ...
                this.$rules = {
                    start: [ {
                        ...
                        stateName: "heredoc",
                        onMatch: function(e, t, n) {
                            var r = "-" == e[2] ? "indentedHeredoc" : "heredoc", s = e.split(this.splitRegex);
                            return n.push(r, s[3]), [ {
                                type: "constant",
                                value: s[1]
                            }, {
            ...
            r.inherits(c, s), t.RubyHighlightRules = c;
        }), ace.define("ace/mode/matching_brace_outdent", [ "require", "exports", "module", "ace/range" ], function(e, t, n) {
            "use strict";
            var r = e("../range").Range, s = function() {};
            (function() {
                this.checkOutdent = function(e, t) {
                    return !!/^\s+$/.test(e) && /^\s*\}/.test(t);
                }, this.autoOutdent = function(e, t) {
                    var n = e.getLine(t), s = n.match(/^(\s*\})/);
                    if (!s) return 0;
                    var a = s[1].length, i = e.findMatchingBracket({
                        row: t,
                        column: a
                    });
                    if (!i || i.row == t) return 0;
                    var o = this.$getIndent(e.getLine(i.row));
            ...
            "use strict";
            var r = e("../../lib/oop"), s = e("./fold_mode").FoldMode, a = e("../../range").Range, i = t.FoldMode = function() {};
            r.inherits(i, s), function() {
                this.getFoldWidgetRange = function(e, t, n) {
                    var r = this.indentationBlock(e, n);
                    if (r) return r;
                    var s = /\S/, i = e.getLine(n), o = i.search(s);
                    if (-1 != o && "#" == i[o]) {
                        for (var c = i.length, l = e.getLength(), _ = n, d = n; ++n < l; ) {
                            i = e.getLine(n);
                            var g = i.search(s);
                            if (-1 != g) {
                                if ("#" != i[g]) break;
                        ...
                        if (d > _) {
                            var u = e.getLine(d).length;
                            return new a(_, c, d, u);
                        }
                    }
                }, this.getFoldWidget = function(e, t, n) {
                    var r = e.getLine(n), s = r.search(/\S/), a = e.getLine(n + 1), i = e.getLine(n - 1), o = i.search(/\S/), c = a.search(/\S/);
                    if (-1 == s) return e.foldWidgets[n - 1] = -1 != o && o < c ? "start" : "", "";
                    if (-1 == o) {
                        if (s == c && "#" == r[s] && "#" == a[s]) return e.foldWidgets[n - 1] = "", e.foldWidgets[n + 1] = "", 
                        "start";
                    } else if (o == s && "#" == r[s] && "#" == i[s] && -1 == e.getLine(n - 2).search(/\S/)) return e.foldWidgets[n - 1] = "start", 
                    e.foldWidgets[n + 1] = "", "";
                    return e.foldWidgets[n - 1] = -1 != o && o < s ? "start" : "", s < c ? "start" : "";
                };
            }.call(i.prototype);
            ...
            "use strict";
            var r = e("../lib/oop"), s = e("./text").Mode, a = e("./ruby_highlight_rules").RubyHighlightRules, i = e("./matching_brace_outdent").MatchingBraceOutdent, o = e("../range").Range, c = e("./behaviour/cstyle").CstyleBehaviour, l = e("./folding/coffee").FoldMode, _ = function() {
                this.HighlightRules = a, this.$outdent = new i(), this.$behaviour = new c(), this.foldingRules = new l();
            };
            r.inherits(_, s), function() {
                this.lineCommentStart = "#", this.getNextLineIndent = function(e, t, n) {
                    var r = this.$getIndent(t), s = this.getTokenizer().getLineTokens(t, e), a = s.tokens;
                    if (a.length && "comment" == a[a.length - 1].type) return r;
                    if ("start" == e) {
                        var i = t.match(/^.*[\{\(\[]\s*$/), o = t.match(/^\s*(class|def|module)\s.*$/), c = t.match(/.*do(\s*|\s+\|.*\|\s*)$/), l = t.match(/^\s*(if|else|when)\s*/);
                        (i || o || c || l) && (r += n);
                    }
                    return r;
                    ...
                    var s = this.$getIndent(r), a = t.getLine(n - 1), i = this.$getIndent(a), c = t.getTabString();
                    i.length <= s.length && s.slice(-c.length) == c && t.remove(new o(n, s.length - c.length, n, s.length));
